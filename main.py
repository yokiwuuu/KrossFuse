...
class KrossFuse:
    ...
    def hadamard_product_fusion(
        ...
        clip_random_matrix = torch.randn(clip_dim, n_components, device=self.device, dtype=torch.float32) / np.sqrt(n_components)
        dino_random_matrix = torch.randn(dino_dim, n_components, device=self.device, dtype=torch.float32) / np.sqrt(n_components)
